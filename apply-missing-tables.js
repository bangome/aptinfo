/**
 * Script to create missing transaction tables in Supabase
 * Run with: node apply-missing-tables.js
 */
...
-- Create apartment_trade_transactions table if not exists
...
-- Create apartment_rent_transactions table if not exists
...
async function createMissingTables() {
  try {
    console.log('🚀 Creating missing transaction tables...');
    
    const { data, error } = await supabase.rpc('exec_sql', {
      sql: createTransactionTablesSQL
    });
    ...
    if (!tradeError && !rentError) {
      console.log('✅ All tables are accessible!');
    } else {
      console.log('⚠️ Some tables may not be accessible:', { tradeError, rentError });
    }
