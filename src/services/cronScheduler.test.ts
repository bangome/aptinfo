...
import { CronScheduler, type ScheduledJob } from './cronScheduler';
import { jest } from '@jest/globals';

// Mock node-cron
jest.mock('node-cron', () => ({
  schedule: jest.fn((expression, task, options) => ({
    start: jest.fn(),
    stop: jest.fn(),
    destroy: jest.fn()
  })),
...
jest.mock('@/services/governmentApiService');
...
describe('CronScheduler', () => {
  ...
  describe('startJob / stopJob', () => {
    ...
    it('존재하지 않는 작업을 시작하려고 하면 false를 반환해야 합니다', () => {
      const result = scheduler.startJob('non-existent-job');
      expect(result).toBe(false);
    });

    it('존재하지 않는 작업을 중지하려고 하면 false를 반환해야 합니다', () => {
      const result = scheduler.stopJob('non-existent-job');
      expect(result).toBe(false);
    });
  });
  ...
  describe('runJobNow', () => {
    ...
    it('존재하지 않는 작업을 실행하려고 하면 에러를 던져야 합니다', async () => {
      await expect(scheduler.runJobNow('non-existent-job')).rejects.toThrow();
    });
  ...
  describe('error handling', () => {
    ...
    it('최대 재시도 횟수를 초과하면 작업이 중지되어야 합니다', async () => {
      const failingTask = jest.fn().mockRejectedValue(new Error('Test error'));
  ...
  describe('removeJob', () => {
    ...
    it('존재하지 않는 작업을 제거하려고 하면 false를 반환해야 합니다', () => {
      const result = scheduler.removeJob('non-existent-job');
      expect(result).toBe(false);
    });
  });
